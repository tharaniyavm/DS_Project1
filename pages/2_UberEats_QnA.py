...
analysis_queries = {
    ...
    """ 7.	Which cuisines are most common in Bangalore?
           ...
           : """
                    SELECT 
                        TRIM(json_each.value) AS cuisine,
                        COUNT(*) AS restaurant_count
                    FROM restaurants,
    ...
    """11.	Which locations are ideal for premium restaurant onboarding?
        ...
        """15.	Which restaurants are top performers within each pricing segment?
            ...
            :""" SELECT * FROM (
                        SELECT 'Low' AS segment, restaurant_name, MAX(rate) as rate, MAX(votes) as votes 
                        FROM restaurants 
                        WHERE approx_two_person_cost <= 500 
                        GROUP BY restaurant_name -- Groups duplicates into one row
                        ORDER BY rate DESC, votes DESC LIMIT 3
                    ) AS low_tier
                    ...
                    SELECT * FROM (
                        SELECT 'Mid' AS segment, restaurant_name, MAX(rate) as rate, MAX(votes) as votes 
                        FROM restaurants 
                        WHERE approx_two_person_cost BETWEEN 501 AND 1500 
                    ...
                    SELECT * FROM (
                        SELECT 'Premium' AS segment, restaurant_name, MAX(rate) as rate, MAX(votes) as votes 
                        FROM restaurants 
                        WHERE approx_two_person_cost > 1500 
                        GROUP BY restaurant_name
                        ORDER BY rate DESC, votes DESC LIMIT 3
                    ) AS premium_tier;""" 
}
